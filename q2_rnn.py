...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        sentence_new = sentence + [zero_vector] * (max_length-sentence_len)
        labels_len = len(labels)
        labels_new = labels + [zero_label] * (max_length-labels_len)
        mask = [True] * labels_len + [False] * (max_length-labels_len)
        ret.append((sentence_new[:max_length], labels_new[:max_length], mask[:max_length]))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        """Adds the unrolled RNN:
            ...
            for t in 1 to T:
                ...
                h_dropout = Dropout(o_t, dropout_rate)
                y_t = h_dropout U + b_2
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                y_t = tf.matmul(h_dropout, U) + tf.expand_dims(b2, 0)
                preds.append(y_t)
                ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        ce_cost = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds)
        loss = tf.reduce_mean(tf.boolean_mask(ce_cost, self.mask_placeholder))
        ### END YOUR CODE
        return loss
