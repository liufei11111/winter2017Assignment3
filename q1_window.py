...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in xrange(window_size):
            sentence_new.append(start)
        sentence_new.extend(sentence)
        for i in xrange(window_size):
            sentence_new.append(end)
        for i in range(length):
            idx = i + window_size
            start_index = idx - window_size
            end_index = idx + window_size + 1
            point = sentence_new[start_index:end_index]
            windowed_data.append(([item for sublist in point for item in sublist], labels[i]))
		### END YOUR CODE
    # print 'windowed_data', windowed_data[:1]
...
class WindowModel(NERModel):
    ...
    def train_on_batch(self, sess, inputs_batch, labels_batch):
        print 'train_on_batch inputs_batch', inputs_batch[0]
        print 'train_on_batch labels_batch', labels_batch[0]
